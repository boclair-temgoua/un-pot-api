...
export class UsersService {
    ...
    async findAll(selections: GetUsersSelections): Promise<any> {
        ...
        //  AND "fol"."organizationId" IN ('${userId}'))
    ...
    async findOnePublicBy(
      ...
      SELECT
          CAST(COUNT(DISTINCT fol) AS INT)
      FROM "follow" "fol"
      WHERE ("fol"."organizationId" = "user"."organizationId"
      AND "fol"."deletedAt" IS NULL)
      GROUP BY "fol"."organizationId", "user"."organizationId"
      ) AS "totalFollowing"`
            )
            .addSelect(
                /*sql*/ `(
      SELECT
          CAST(COUNT(DISTINCT fol) AS INT)
      FROM "follow" "fol"
      WHERE ("fol"."followerId" = "user"."organizationId"
      AND "fol"."deletedAt" IS NULL)
      GROUP BY "fol"."followerId", "user"."organizationId"
      ) AS "totalFollower"`
            )
            .addSelect(
                /*sql*/ `(
      ...
      FROM "subscribe" "sub"
      WHERE ("sub"."subscriberId" = "user"."organizationId"
      ...
      AND "sub"."deletedAt" IS NULL)
        ...
        AND "tran"."organizationSellerId" = "user"."organizationId"
        GROUP BY "tran"."organizationSellerId", "user"."organizationId"
        ) AS "donation"`
            )
            .addSelect(
                /*sql*/ `(
        ...
        GROUP BY "mem"."organizationId", "user"."organizationId"
            ...
            .leftJoin('organization.donation', 'donation')
      ...
      WHERE ("fol"."followerId" = "user"."organizationId"
       AND "fol"."deletedAt" IS NULL
       AND "fol"."organizationId" IN ('${followerId}'))
       GROUP BY "fol"."organizationId", "user"."organizationId"
      ) AS "isFollow"`);
        }
    ...
    async findOneInfoBy(
        ...
        let query = this.driver
          ...
          'currency', jsonb_build_object(
            ...
            FROM "product" "pro"
            WHERE ("pro"."organizationId" = "user"."organizationId"
            AND "pro"."deletedAt" IS NULL)
            GROUP BY "pro"."organizationId", "user"."organizationId"
            ) AS "totalProduct"`
            )
            .addSelect(
                /*sql*/ `(
