...
export class TransactionsController {
    ...
    async findAll(
        ...
        const { user } = req;
        const { model, organizationBuyerId, organizationId, days } = query;
        ...
        const transactions = await this.transactionsService.findAll({
            ...
            pagination,
            organizationSellerId: organizationId ?? user.organizationId,
        });
    ...
    async findGroupOrganization(
        ...
        const transactions =
            await this.transactionsService.findGroupOrganization({
                organizationSellerId: organizationId ?? user.organizationId,
                days: Number(days) > 0 ? Number(days) : null,
            });
