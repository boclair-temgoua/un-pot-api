...
export class PostsService {
    ...
    async findAll(
        ...
        if (subscribeOrganizationId) {
            query = query.addSelect(/*sql*/ `(
              SELECT
                  CAST(COUNT(DISTINCT sub) AS INT)
              FROM "subscribe" "sub"
              WHERE ("sub"."subscriberId" = "post"."organizationId"
              AND "sub"."deletedAt" IS NULL
              AND "sub"."expiredAt" >= now()::date
              AND "sub"."organizationId" IN ('${subscribeOrganizationId}'))
              ) AS "isValidSubscribe"`);
        } else {
        ...
        if (followerIds && followerIds.length > 0) {
            query = query.andWhere('post.organizationId IN (:...followerIds)', {
                followerIds,
            });
        }
