...
export class CartsService {
    ...
    async findAll(selections: GetCartsSelections): Promise<CartResponse> {
        ...
        let query = this.driver
            ...
            .addSelect('cart.organizationBuyerId', 'organizationBuyerId')
        ...
        if (ipLocation) {
            query = query.andWhere('cart.ipLocation = :ipLocation', {
                ipLocation,
            });
        }
        ...
        if (organizationBuyerId) {
            query = query.andWhere(
                'cart.organizationBuyerId = :organizationBuyerId',
                {
                    organizationBuyerId,
                }
            );
        ...
        const totalPriceDiscount = carts?.reduce(
            (acc, item) =>
                acc +
                Number(item?.product?.priceDiscount) * Number(item?.quantity),
            0
        );
        ...
        return {
            summary: {
                ...
                organizationBuyerId: carts[0]?.organizationBuyerId ?? '',
            },
            cartItems: carts,
        };
    }
